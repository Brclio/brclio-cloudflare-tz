...
const [css, js, captures, cloudflareCaptures, font, brand, license] = await Promise.all([
...
for (const entry of [...manifest.images, ...cloudflare.images]) {
  ...
  images.set(name, { ...entry, annotated: name.startsWith('cf-'), data: `data:image/png;base64,${bytes.toString('base64')}` });
}
...
await writeFile(path.join(root,guide.manifest),JSON.stringify({
  ...
  annotatedImages:cloudflare.images.filter(entry => used.has(path.basename(entry.file,'.png'))).map(({file,width,height,bytes,sha256}) => ({file,width,height,bytes,sha256})),
  sourceFiles:[guide.source,'docs/tutorial-src/styles.css','docs/tutorial-src/app.js','scripts/build-tutorial.mjs'],
  selfContained:true, screenshotProcessing:'Local application PNGs unchanged. User Cloudflare screenshots cropped at native scale, credentials removed and instructional annotations added before embedding. See cloudflare-captures.json.'
},null,2)+'\n');
