...
需要离线阅读时，两篇教程均提供内嵌图片与字体的独立 HTML，可在仓库文件页选择 **Download raw file** 后打开；放在同一目录时可互相跳转。当前仓库版新增 28 张高清操作标注图，支持点击放大、原始尺寸查看和图片下载；也可下载下方 v1.0.4 的教程合集 ZIP，解压后打开。
...
当前 **v1.0.8 源码**在协议首包、HTTP 测速端口、代理握手与 DNS 修复之上，将随机订阅候选默认增至 64 个、上限增至 1000 个，并避免重复 IP。下方 v1.0.4 附件不包含这些改进；从源码构建可获得当前完整部署包。与原版的逐项对照、故障复现和已知限制见 [代理深度排查](docs/proxy-deep-audit.md)，实测与部署后验收方法见 [代理性能说明](docs/proxy-performance.md)。源码测试通过不代表已完成公网部署或证明带宽提升。
...
部署 v1.0.8 或以上源码后，在 **订阅管理 → 订阅来源** 选择「本地地址库」、开启「随机 IP」，将「随机节点数量」设为需要的数量并保存主配置，再在 Clash Verge 等客户端更新订阅。

- 新安装默认 **64 个**，可设置 **1–1000**；面板提供 64 / 128 / 256 / 512 / 1000 快捷选择。
- 升级保留已保存的数量，包括旧默认 16；已有部署需在面板调整一次并保存。
- 每次请求订阅时按当前配置生成一批不同的候选 IP，地址池不足则返回池内全部不同 IP。重复、重叠网段不会重复占用名额；源站失败或没有合法 IPv4 网段时仍使用内置 Cloudflare 地址池。
- 数量设置只作用于随机生成；自定义地址列表和远程订阅生成器继续使用各自提供的节点。

这些是候选地址，增加数量提供更多选择，不等同于速度或可用性提升。更多节点会增加客户端批量测速时间，可先用 128 或 256 个。Cloudflare 收到订阅请求才生成数据；客户端仍需手动或自动更新订阅，服务端没有新增定时刷新任务。
