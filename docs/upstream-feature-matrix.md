...
> 2026-09-17 重新下载实际上游源码与管理页核对，2026-09-18 复核上游 main 未变。当前源码为 v1.0.7，下方保留 v1.0.4 面板核对记录。**“已实现”不等于所有客户端与第三方服务均已在线验收**。公网部署、真实凭据与客户端验收边界见 [validation.md](validation.md)。

## v1.0.7 代理深度排查

对照固定上游源码，修复协议头分片、HTTP CONNECT / SOCKS5 握手、明确出口路由及 DNS 期限与 TTL。常规功能矩阵不覆盖全部网络生命周期；已复现但未修复的 UDP DNS、gRPC 半关闭、SS 跨记录地址，以及自定义 TLS / TURN 长连接限制，见 [完整审计](proxy-deep-audit.md)。

## v1.0.6 测速回退兼容修复

HTTP 80 目标使用无端口 ProxyIP 时，默认回退端口改为 80，避免明文发往 443 产生 HTTP 400；显式端口和其他目标的既有缺省保留。指定运营商参数在 Clash 转换回源中不再被覆盖。已对现有部署进行同入口、同目标的有限对照；不把短时延迟或本地测试当作带宽提升证明，详情见 [代理性能说明](proxy-performance.md)。
