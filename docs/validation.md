...
本轮没有执行 Cloudflare 部署或用户客户端更新。随机候选数量不代表可连通数量、延迟或带宽；公共转换器的容量限制与生产订阅验收仍需部署后确认。
...
- 在用户已有的两个部署间，以隔离的 Mihomo v1.19.29 进程固定 4 组入口 IP / 端口、WS / TLS 与统一延迟，交替测量。gstatic HTTP 每侧 12 次均返回延迟，原版中位数 193.5 ms，项目中位数 214.5 ms；另用 HTTP 客户端核验两者目标均返回 204。这不是带宽测试或长期速度排名。
- 核对 Clash Verge 2.5.2 对应版本源码，手动选择组未配置 URL 时默认访问 `http://cp.cloudflare.com/generate_204`，与自动选择组中的 gstatic URL 不同。
- 同入口 cp HTTP 对照每侧 8 次，原版中位数 192 ms，项目中位数 1047.5 ms。额外读取响应后确认：原版为本地 204，项目为实际 HTTP 400，正文为明文 HTTP 被送到 HTTPS 端口。Mihomo 的延迟数字不能单独当成目标成功的证据。
- 在临时节点路径中明确指定同区域 ProxyIP 的 80 端口后，获得真实 Cloudflare 204。未改用户正在运行的 Clash 配置或订阅；隔离核心及含凭据的临时配置已清理。没有发布用户的节点身份、订阅令牌或控制器凭据。
- 新增 12 项生产 bundle / workerd 回归：强制直连失败后通过真实本机 HTTP 服务完成 ProxyIP 回退，涵盖 IPv4、IPv6、域名 A、TXT、显式端口、父级端口继承及 TXT 端口优先级。同一连接两次 HEAD 的 204 带服务器随机响应标识，避免被 Worker 本地短路误判为成功。443 / 8443 目标仍以默认 443 原样转发 TLS 记录；这一用例不是完整 TLS 握手。
- 用保留的 v1.0.5 构建产物运行其中 11 项：4 个隐式 HTTP 端口用例均收到真实 400 而失败，其余 7 项兼容检查通过。修复版全部通过。
- 新增 3 项订阅集成测试，实际执行 Clash 转换 URL 构造、境外转换器回源与 mixed 输出，覆盖显式运营商、大小写 / 空白归一化、非法参数回退、ASN / 组织名识别；未引入额外公网转换请求。

完整诊断、测速设置建议与候选池固定方法见 [proxy-performance.md](proxy-performance.md)。v1.0.6 的线上部署及部署后的客户端验收尚未执行；临时端口实验不能替代它。
